...
const AddCategoryModal = (props) => {
  ...
  const onChangeImage = async(img, next) => {
		const file = img;
		console.log(file);
		let url;
		if (!file) return;

		const storageRef = projectStorage.ref(`Category/${file.name}`);
  ...
  const submitForm = async (e) => {
    ...
    await storageRef.put(file).on('state_changed', (snapshot)=> {
      ...
      console.log(err);
        ...
        try {
          let responseData = await createCategory(fData, url);
