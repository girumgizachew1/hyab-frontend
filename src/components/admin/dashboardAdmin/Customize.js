...
const UploadImageSection = () => {
  ...
  let newImage;
  const [reiszeImage, setReiszeImage] = useState();

  const uploadImageHandler = async (image) => {
    const storageRef = projectStorage.ref(`Customizer/${image.name}`);
    const collectionRef = projectFirestore.collection("Customizer");
        ...
        try {
          ctx.canvas.toBlob(
            (blob) => {
              ...
              setReiszeImage(fileNew);
              console.log(reiszeImage);
