...
export const forgePasswordReq = async ({ email}) => {
  const data = {email};
  try {
    let res = await axios.post(`${apiURL}/api/password`, data);
    return res.data;
  } catch (error) {
    console.log(error);
  }
...
export const changePasswordReq = async ({url, password}) => {
  const data = {url, password};
  try {
